...
import { createPurchaseDocumentService } from "../services/purchaseDocumentService.js";

export const createPurchaseDocumentController = (typeKey) => {
  const service = createPurchaseDocumentService(typeKey);
  const { label } = service.config;
  const fileSlug =
    typeKey === "commandeFournisseur" ? "bon-commande-fournisseur" : typeKey;
...
export default createPurchaseDocumentController;
