...
const buildFromExisting = (line) => {
  ...
  const prixAchat = parseFloat(line.prixAchat ?? 0);
  const familyRemise = parseFloat(line.familyRemise ?? line.family?.remise ?? 0);
  const discountDecimal = parseFloat(line.remise ?? line.discount ?? 0);
  const discountPct = discountDecimal <= 1 ? discountDecimal * 100 : discountDecimal;
  return {
    lineId: line.id,
    type: line.variantId ? "variant" : "article",
    id: line.variantId ?? line.articleId,
    variantId: line.variantId ?? null,
    articleId: line.articleId ?? null,
    name: line.article?.name ?? line.variant?.name ?? line.description ?? "—",
    barcode: line.article?.barcode ?? line.variant?.barcode ?? "—",
    familyName: line.article?.family?.name ?? line.variant?.article?.family?.name ?? null,
    prixAchat,
    unitPrice,
...
const EditableCell = ({ value, onActivate, onCommit, min = 0, max, step = 1, width = "w-20", className = "" }) => {
  ...
  const handleChange = (e) => {
    ...
    else { let parsed = parseFloat(e.target.value); if (isNaN(parsed)) parsed = 0; onActivate?.(); onCommit(parsed); }
  };
  return (
    <input type="number" min={min} max={max} step={step}
      value={editing ? raw : value}
      onFocus={handleFocus} onBlur={handleBlur} onChange={handleChange}
      className={`${width} px-2 py-1.5 border rounded-lg text-center text-sm font-bold outline-none focus:border-[#B12B89] focus:ring-2 focus:ring-[#B12B89]/15 transition-colors dark:text-slate-100 bg-white dark:bg-[#222222] border-slate-300 dark:border-[#2e2e2e] ${className}`}
    />
  );
};
...
/* ─────── BonLivraisonEditForm ─────── */
export const BonLivraisonEditForm = () => {
  ...
  const updateMutation = useUpdateBonLivraison();
  ...
  const { data: blData, isLoading: blLoading, isError } = useBonLivraisonById(id);
  ...
  const { data: clientsData, isLoading: clientsLoading } = useBLClients({ societeId });
  const { data: deliveriesData } = useDeliveries(isSuperAdmin ? { societeId: societeId ?? undefined } : {});
  const deliveries = deliveriesData?.data ?? [];

  // ✅ NEW: fetch advances when client + societe are set
  ...
  /* draft logic */
  const BL_EDIT_DRAFT_KEY = "bl_edit_draft_state";
  const editReturnTo = `/bon-livraisons/${id}/edit`;

  const handleSaveAndNavigateToClient = () => {
    const draft = { documentDate: documentDate?.format("YYYY-MM-DDTHH:mm:ss"), dateLivraison, selectedDepot, selectedClient, selectedDelivery, status, notes, priceField, selectedProducts, selectedAdvanceIds, selectedPacks };
    sessionStorage.setItem(BL_EDIT_DRAFT_KEY, JSON.stringify(draft));
    navigate(`/clients/create?returnTo=${encodeURIComponent(editReturnTo)}`);
  };

  const handleSaveAndNavigateToArticle = () => {
    const draft = {
      documentDate: documentDate?.format("YYYY-MM-DDTHH:mm:ss"), dateLivraison, selectedDepot, selectedClient,
      selectedDelivery, status, notes, priceField, selectedProducts,
      selectedAdvanceIds, selectedPacks,
    };
    sessionStorage.setItem(BL_EDIT_DRAFT_KEY, JSON.stringify(draft));
  ...
  useEffect(() => {
    const saved = sessionStorage.getItem(BL_EDIT_DRAFT_KEY);
    if (!saved) return;
    try {
      ...
      sessionStorage.removeItem(BL_EDIT_DRAFT_KEY);
  ...
  /* Pre-fill from fetched BL */
  useEffect(() => {
    if (initialized || blLoading || !blData || depots.length === 0) return;
    const bl = blData?.data ?? blData;
    const doc = bl?.document ?? bl;

    if (bl?.documentDate) setDocumentDate(dayjs(bl.documentDate));
    if (bl?.dateLivraison) setDateLivraison(dayjs(bl.dateLivraison).format("YYYY-MM-DD"));
    if (doc?.status) setStatus(doc.status);
    if (doc?.notes) setNotes(doc.notes ?? "");

    const depot = depots.find((d) => d.id === bl?.depotId);
    if (depot) setSelectedDepot(depot);

    if (bl?.deliveryId) {
      const delivery = deliveries?.find((d) => d.id === parseInt(bl.deliveryId));
      if (delivery) setSelectedDelivery(delivery);
    }

    if (bl?.priceField) setPriceField(bl.priceField);

    const lines = doc?.lines ?? bl?.lines ?? [];
    if (lines.length > 0) setSelectedProducts(lines.map(buildFromExisting));

    const packLines = doc?.packLines ?? bl?.packLines ?? [];
    if (packLines.length > 0) setSelectedPacks(packLines.map(buildFromExistingPackLine));

    // ✅ Pre-fill advanceIds from existing BL
    if (Array.isArray(bl?.advanceIds) && bl.advanceIds.length > 0) {
      setSelectedAdvanceIds(bl.advanceIds);
    } else if (Array.isArray(doc?.advanceIds) && doc.advanceIds.length > 0) {
      setSelectedAdvanceIds(doc.advanceIds);
    }

    setInitialized(true);
  }, [blData, blLoading, depots, initialized]);

  /* Set client after societeId filter loads clients */
  useEffect(() => {
    if (!initialized || !blData || clients.length === 0 || selectedClient) return;
    const bl = blData?.data ?? blData;
    const doc = bl?.document ?? bl;
    const clientId = doc?.clientId ?? bl?.clientId;
    if (clientId) {
      const client = clients.find((c) => c.id === clientId);
      if (client) setSelectedClient(client);
    }
  }, [clients, initialized, blData, selectedClient]);
  ...
  const handleSubmit = async () => {
    ...
    const payload = {
      ...(isDraft && { clientId: selectedClient.id, depotId: selectedDepot.id }),
    ...
    if (status === "DRAFT") {
      payload.lines = selectedProducts
      ...
      payload.packLines = selectedPacks.map((p) => {
        const line = {
          packId: p.packId,
          quantity: p.quantity,
          prixVente: p.prixVente,
        };
        if (p.packLineId) line.id = p.packLineId;
        return line;
      });
    }
    ...
    updateMutation.mutate(
      { id, payload },
      {
        onSuccess: (res) => { toast.success(res?.message || t("toast.update_success")); navigate("/bon-livraisons"); },
  ...
  if (blLoading || !initialized) return <SectionLoader />;

  if (!blData?.data || isError) {
    return <NotFound onAction={() => navigate("/bon-livraisons")} />;
  }

  const bl = blData?.data ?? blData;
  const docNumber = bl?.document?.documentNumber ?? bl?.documentNumber ?? `BL #${id}`;
            ...
            <div className={!isDraft ? "pointer-events-none opacity-50 space-y-5" : "space-y-5"}>
              <FormGroup title={tCommon("form_group_parties")}>
                <FormFieldGrid cols={3}>
                    ...
                    {isDepotLocked ? (
                      <>
                        <div className="relative">
                          <input readOnly value={`${selectedDepot?.name} — ${selectedDepot?.societe?.raisonSocial ?? ""}`}
                        ...
                        <p className="text-xs text-amber-500 mt-1.5">{t("remove_articles_to_change_depot_hint")}</p>
                      </>
                    ) : (
                      <SelectDropDown placeholder={t("select_depot")} value={selectedDepot?.id || ""}
                  ...
                  <div>
                    {!selectedDepot ? (
                      <div>
                        <label className={FORM_LABEL}>{t("delivery_person_label")}</label>
                        <div className={FORM_CONTROL_DASHED}>
                          {t("select_depot_first")}
                        </div>
                      </div>
                    ) : (
                      <SelectDropDown label={t("delivery_person_label")} placeholder={t("select_delivery_person")} value={selectedDelivery?.id || ""}
                        options={deliveries?.map((d) => ({ value: d.id, label: d.name, subLabel: d.type })) || []}
                        onChange={(e) => { const delivery = deliveries?.find((d) => d.id === parseInt(e.target.value)); setSelectedDelivery(delivery || null); }} />
                    )}
                  </div>
                </FormFieldGrid>
              </FormGroup>
              ...
              <FormGroup title={tCommon("form_group_dates_status")}>
                <FormFieldGrid cols={4}>
                  <div>
                    <label className={FORM_LABEL}>{t("document_date")}</label>
                        ...
                        <tr key={`${productKey(product)}-${product.lineId ?? "new"}`}
                              ...
                              <div>
                                ...
                                {hasLowStock && (
                                  <div className="flex items-center gap-1 mt-0.5">
                                    ...
                                    <span className="text-[9px] text-amber-600 dark:text-amber-400 font-bold">{t("stock_warning", { available: product.stock?.quantityAvailable ?? 0 })}</span>
                                  </div>
                                )}
                              </div>
                            </div>
                          ...
                          {isDraft && (
                            <td className="px-6 py-4 text-right">
                              <button onClick={() => handleRemoveProduct(product)} className="p-1.5 rounded-lg text-slate-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 transition-all">
