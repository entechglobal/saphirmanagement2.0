...
import { useBLProducts } from "../hooks/useBonLivraisons";
...
export const SelectBLProductsModal = ({
  isOpen,
  depotId,
  ...
  const { data: productsData, isLoading, isFetching } = useBLProducts({
    depotId,
  ...
  // When the API returns fresh product data (after price field change or page change),
  // sync pending products that appear in the current page with the updated prices.
  useEffect(() => {
  ...
  const inputClass = "w-full px-4 py-2.5 bg-white dark:bg-[#222222] border border-slate-200 dark:border-[#2e2e2e] rounded-xl focus:ring-2 focus:ring-[#B12B89] outline-none transition dark:text-slate-100 text-sm";
