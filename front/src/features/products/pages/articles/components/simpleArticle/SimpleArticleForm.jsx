...
export const SimpleArticleForm = ({ id, initialValues, mode = "create", createMode, returnTo, onDirtyChange }) => {
  ...
  const isDirty = isEditMode
    ? Boolean(
        initialFormData &&
        Object.entries(initialFormData).some(([k, v]) => {
          if (k === "image") return false;
          return String(formData[k] ?? "") !== String(v ?? "");
        })
      )
    : !!(formData.name || formData.barcode || formData.prixAchat || formData.prixVente1);

  // Only register a blocker in edit mode — the parent page owns the blocker in create mode.
  // Passing `false` to useBlocker tells React Router not to register it at all,
  // avoiding the "only one blocker at a time" warning.
  const blockNavigation = useCallback(
    ({ currentLocation, nextLocation }) =>
      isDirty && !savingRef.current && currentLocation.pathname !== nextLocation.pathname,
    [isDirty]
  );
  const blocker = useBlocker(createMode ? false : blockNavigation);
  ...
  useEffect(() => {
    if (isEditMode && data) {
      const initial = {
        ...
        unitePrincipaleId: data.unitePrincipaleId || "",
        uniteSecondaireId: data.uniteSecondaireId || "",
        uniteComplementaireId: data.uniteComplementaireId || "",
        conversionSecondaire: data.conversionSecondaire || "",
        conversionComplementaire: data.conversionComplementaire || "",
        ...
        dureeExpiration: data.dureeExpiration || "",
        remise: data.remise ? Math.round(data.remise * 100) : "0",
      };

      setFormData({ ...initial, image: null });
      setInitialFormData(initial);

      if (data.image) setImagePreview(data.image);
      if (data.uniteSecondaireId) setShowSecondaryUnit(true);
      if (data.uniteComplementaireId) setShowComplementaryUnit(true);
    }
  }, [isEditMode, data]);

  useEffect(() => {
    if (isEditMode || units.length === 0) return;
  ...
  const handleSubmit = async (e) => {
    ...
    Object.entries(finalFormData).forEach(([key, value]) => {
      ...
      if (isEditMode && initialFormData) {
        if (value === initialFormData[key]) return;
      }
    ...
    // force numeric fields if present
    if (fd.has("familyId"))
      fd.set("familyId", Number(formData.familyId));

    if (fd.has("unitePrincipaleId"))
    ...
    // nothing changed
    if (isEditMode && [...fd.keys()].length === 0) {
    ...
    try {
      if (isEditMode) {
        await updateMutation.mutateAsync({ id, payload: fd });
        ...
        if (returnTo) {
          navigate(
            `/articles/create/simple?articleId=${createdArticle.data.id}&returnTo=${encodeURIComponent(returnTo)}`
          );
          return;
        }
        navigate(`/articles/create/simple?articleId=${createdArticle.data.id}`);
      }

    } catch (err) {
      const msg = err?.response?.data?.message;
      toast.error(msg || (isEditMode ? t("toast.update_error") : t("toast.create_error")));
    }
  };
  ...
  return (
    <>
    <div className={createMode ? "" : "-m-4 lg:-m-6 px-3 pt-3"}>
      {/* HEADER SECTION */}
      {!createMode && (
        <FormPageHeader
          entityName={t("title")}
          ...
          backLabel={t("create_mode.back_to_list")}
          onBack={() => {
            if (isDirty) setPendingBack(true);
            else { savingRef.current = true; navigate(returnTo || "/articles"); }
          }}
        />
      )}
