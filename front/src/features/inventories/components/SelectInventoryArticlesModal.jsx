...
export const SelectInventoryArticlesModal = ({
  isOpen,
  articles,
  isLoading,
  onClose,
  ...
  const handleToggle = (article) => {
    ...
    if (isPending(article)) {
      ...
      setPendingArticles((prev) => [...prev, { ...article, quantityCounted: 0, isTouched: false }]);
    }
  };
  ...
  const handleToggleAll = () => {
    ...
    if (allPending) {
      ...
      const toAdd = eligible.filter((a) => !isPending(a)).map((a) => ({ ...a, quantityCounted: 0, isTouched: false }));
  ...
  return (
    <BaseModal
      ...
      subtitle={`${selectedDepot?.name ? `${selectedDepot.name} · ` : ""}${pagination.totalItems ?? 0} ${t("available_articles")}${pendingArticles.length > 0 ? ` · ${pendingArticles.length} ${t("pending_add")}` : ""}`}
