...
import {
  useReglementClients,
  useDeleteReglementClient,
  useRCClients,
  usePrintReglementClient,
} from "../hooks/useReglementClient";
...
const MODE_CONFIG = {
  ...
  CARTE_FIDELITE:  { color: "bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-900/20 dark:text-amber-300 dark:border-amber-800" },
};
...
export const ReglementClientsPage = () => {
  ...
  useEffect(() => {
    console.log("[ReglementClientsPage] Time filter changed:", {
  ...
  const { data, isLoading, isFetching, isError } = useReglementClients({
  ...
  const deleteMutation = useDeleteReglementClient();
  ...
  const handlePrint = (row) => {
    ...
    printMutation.mutate(row.id, {
      onError: (err) => {
        toast.error(err?.response?.data?.message || t("print_error"));
      },
    });
  };
    ...
    },
    {
      ...
      Cell: ({ cell }) => (
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm">{cell.getValue() ? cell.getValue() : "—"}</span>
        </div>
      ),
    },
    {
  ...
  return (
    <div className="p-4 md:p-8 min-h-screen transition-colors duration-300">
      <HeaderTable
        title={t("page_title")}
        onCreate={() => navigate("/reglements-client/create")}
      ...
      <ReusableTable
        ...
        tableId="reglements-client-table"
      />
