...
import {
  ...
  FormFieldGrid,
  FormShell,
} from "../../../shared/components/FormCard";
...
import { CitySearchDropdown } from "../../saphirmanagement/commandes/components/CitySearchDropdown";
...
export const ClientsForm = () => {
  ...
  const {
    ...
    isLoading,
    isError,
  } = useClient(id, { enabled: isEditMode });
  ...
  useEffect(() => {
    if (client?.data) {
      const sanitizedData = Object.entries(client.data).reduce(
        (acc, [key, value]) => {
  ...
  const onSubmit = (formData) => {
    if (user?.isSuperAdmin && !formData.societeId) {
      setError("societeId", {
        type: "manual",
        message: t("messages.societe_required"),
      });
      return;
    }

    const mutation = isEditMode ? updateMutation.mutate : createMutation.mutate;
    let payloadValues = { ...formData, city: getCityName(formData.city) };
    ...
    mutation(payload, {
      onSuccess: (res) => {
        ...
        navigate(
          !isEditMode && returnTo ? decodeURIComponent(returnTo) : "/clients",
        );
      },
  ...
  if (isEditMode && (!client?.data || isError)) {
    return <NotFound onAction={() => navigate("/clients")} />;
  }
          ...
          <div className="space-y-8">
            <div className=" space-y-5">
              <FormCard title={t("sections.identification")}>
                <FormFieldGrid className="grid-cols-1 lg:grid-cols-3 gap-5">
                  {user?.isSuperAdmin && (
                    <Controller
                      control={control}
                      name="societeId"
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <CitySearchDropdown
                        ...
                        value={field.value}
                        onChange={(e) =>
                          field.onChange(getCityName(e.target.value))
                        }
                ...
                <FormCard title={t("sections.fiscal")}>
                  <FormFieldGrid>
                    <Input
                      ...
                      {...register("ice")}
                      placeholder={t("fields.ice.placeholder")}
                      error={errors.ice?.message}
                      maxLength={15}
                    />
                    <Input
                      label={t("fields.if.label")}
                      {...register("if")}
                      placeholder={t("fields.if.placeholder")}
                      error={errors.if?.message}
                    />
                    <Input
                      label={t("fields.rc.label")}
                      {...register("rc")}
                      placeholder={t("fields.rc.placeholder")}
            ...
            {clientType === "SOCIETE" && <hr />}

            <div>
              <FormCard title={t("sections.financial")}>
                <div className="grid gap-5 grid-cols-3">
                  <Input
                    label={t("fields.creditLimit.label")}
                    type="number"
                  ...
                  />
                  <Input
                    label={t("fields.paymentDeadline.label")}
                    type="number"
                    {...register("paymentDeadline", { valueAsNumber: true })}
                    placeholder={t("fields.paymentDeadline.placeholder")}
              ...
              <FormCard title={t("sections.status")} className="mt-6">
                <Controller
                  control={control}
                  name="active"
                  render={({ field }) => (
          ...
          <FormActions
            ...
            submitLabel={
              createMutation.isPending || updateMutation.isPending
                ...
                : isEditMode
                  ? t("actions.submit_update")
                  : t("actions.submit_create")
            }
          />
        </form>
      </FormShell>
