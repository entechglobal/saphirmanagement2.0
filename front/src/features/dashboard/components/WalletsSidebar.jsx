    ...
    <div>
      <div className="mb-1 flex items-center justify-between gap-2 px-1">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="rounded-full bg-slate-100 px-1.5 py-0.5 text-[10px] font-bold tabular-nums text-slate-500 dark:bg-slate-700 dark:text-slate-300">
            {wallets.length}
          </span>
        </div>
      </div>
...
export const WalletsSidebar = () => {
  ...
  return (
    <aside className="flex w-full shrink-0 flex-col overflow-hidden rounded-xl border border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-800 xl:sticky xl:top-8 xl:max-h-[calc(100vh-4rem)] xl:w-[340px]">
      <div className="border-b border-slate-100 px-4 py-3 dark:border-slate-700">
        ...
        <div className="relative mt-2.5">
          ...
          <input
            ...
            className="h-8 w-full rounded-lg border border-slate-200 bg-slate-50 pe-3 ps-8 text-xs text-slate-800 outline-none placeholder:text-slate-400 focus:border-[#B12B89] dark:border-slate-600 dark:bg-slate-900/40 dark:text-slate-100"
          />
        </div>
      </div>
          ...
          Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="flex animate-pulse items-center gap-2.5 px-2 py-2">
              ...
              <div className="flex-1 space-y-1.5">
                ...
                <div className="h-2.5 w-1/3 rounded bg-slate-100 dark:bg-slate-700/70" />
              </div>
            </div>
          ))
        ) : (
